...
- [這個 repo 是什麼](#這個-repo-是什麼) — 節錄範圍、怎麼跑測試、八個模組各在做什麼
- [專案規模](#專案規模) — 完整專案的行數、測試數、支援版本
- [五個值得講的決策](#五個值得講的決策)
  1. [零相依的 xlsx 寫入器](#1-零相依的-xlsx-寫入器) — 難的不是寫 XML，是驗證
  2. [先量再改：匯出快一倍](#2-先量再改匯出快一倍) — 推估錯了 25 倍，111 分鐘降到 64 分鐘
  3. [用像素證據定位缺陷](#3-用像素證據定位缺陷一次-revit-都沒開) — 一次 Revit 都沒開
...
- **完整專案**：21,404 行（不計空白行），依賴 Revit API 才能編譯。它的程式碼不公開，需要閱覽請[聯絡索取](#授權與索取)。
  **這裡節錄的八個模組、它們的測試與所有量測數據則全文公開**——clone 下來就能自己跑一次。
...
需求是匯出兩張工作表：可排序篩選的資料表，加上每筆一張 3D 截圖的圖片表。

不用 ClosedXML／OpenXML SDK——那要往安裝包再塞六到八個 DLL，而**增益集與其他廠商的外掛跑在同一個
Revit 行程裡**，版本衝突的症狀極難查。使用者機器上裝了哪些外掛不是我能控制的。
